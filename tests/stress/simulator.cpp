...
// void doSimulation();
/**
 * This test is inteded to do game simulations and reveal either conccurency
 * problems or memory problems. As well as profile different implementaitons.
 */
int main()
{
    srand(time(NULL));
    ...
    std::vector<std::string> games{
        "tests/stress/selectedGames/connect4.kif",
        "tests/stress/selectedGames/ticTacToe.kif",
        "tests/stress/selectedGames/bomberman2p.kif",
        "tests/stress/selectedGames/knightsTour.kif",
        "tests/stress/selectedGames/cephalopodMicro.kif",
        "tests/stress/selectedGames/chess.kif"};
    perf.setf(std::ios::unitbuf);

    for (size_t i = 0; i < games.size(); i++) profile(games[i], ares);
    return 0;
}
...
uint doSimulation(ares::Ares& ares)
{
    ...
    while (not done) {
        ...
        // std::cout << "Time for a simulation "<< sims <<" " << dur.count() <<
        // "seconds.\n";
        if (state != &init)
            delete state;
        total_steps += steps;
