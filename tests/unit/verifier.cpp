...
void verifyMatches(std::vector<const char*> matchFiles)
{
    ...
    // The reasoner
    Reasoner& reasoner(
        Reasoner::create(*parser, Prover::create(), *mempool.getCache()));
    ...
    for (auto&& matchF : matchFiles) {
        ...
        BOOST_FOREACH (auto& match1, pt) {
            ...
            if (match1.first.find("chess") != string::npos)
                ...
                name = SELECTED_DIR + mappings.get<string>(
                                          ptree::path_type(match1.first, '|'));

            std::cout << "Using GDl file : " << name << "\n";
            std::cout << "Verifying Reasoner with Match: "
                      << match1.second.get<string>("url") << "\n";
            parser->parse(g, name);
            reasoner.reset(g);
            simulateMatch(match, reasoner);
            log("Successfuly Verified Reasoner With Match: ")
                << match1.second.get<string>("url") << "\n";
        }
    }
}
...
void simulateMatch(ptree& match, ares::Reasoner& reasoner)
{
    ...
    for (auto&& moves : matchMoves) {
        ...
        for (size_t i = 0; i < moves.size(); i++) {
            ...
            auto it =
                find(computedMoves->begin(), computedMoves->end(), moves[i]);
    ...
    assert_true(matchState == (*computedState));
    if (computedState != &reasoner.init())
        delete computedState;
    for (auto&& state : matchStates) delete state;
}
/**
        ...
        BOOST_FOREACH (auto& smoves, pt) {
            ...
            BOOST_FOREACH (auto& move, smoves.second) {
                ...
                if (move_s[0] == '(')
                    ...
                    moves_i.push_back(
                        Ares::memCache->getConst(Namer::id(move_s)));
            }
...
std::vector<std::vector<const Term*>*> getState(ptree& pt)
{
    ...
    if (states.size() == 0) {
        BOOST_FOREACH (ptree::value_type& x, pt) {
            auto seq =
                parser->parseSeq(x.second.get_value<std::string>().c_str());
...
void writeOutMatch()
{
    ...
    for (auto&& i : selected) std::cout << "Selecting matche " << i << "\n";

    auto match = getMatch(selected);
    if (not match.size()) {
        std::cout << "Couldn't find a match with game name "
                  << "chess"
                  << "\n";
        return;
    }
        ...
        try {
            ...
            if (it != selected.end()) {
                if (not pt.get<bool>("data.isCompleted")) {
                    uint j = 1;
                    while (std::find(selected.begin(), selected.end(), i + j) !=
                           selected.end())
                        j++;
                    *it = i + j;
                    continue;
