...
namespace ares
{
    ...
    :monte( (Montecarlo&)Registrar::get("Montecarlo")), reasoner(reasoner_),e2(rd())
    {}
    std::vector<Montecarlo::Node*> MonteTester::TestExpansion(MockReasoner::GameTree& game,Montecarlo::Node* node,MockReasoner::Node& mcRNode){
        ...
        for (auto &&[action, child] : mcRNode.get()){
            ...
            for (auto &&c : node->children)
            {
                ...
                if( origA == action ){ found=true; break;}
            }
            if( not found )
    ...
    void MonteTester::TestSelection(){
        ...
        typedef std::function<void(Montecarlo::Node*, std::reference_wrapper<MockReasoner::Node>)> Fn;
        using McRNode = MockReasoner::Node;
        using Node    = Montecarlo::Node;
        ...
        auto testBestSelection = [&](Node* parent, const std::vector<Node*>& children){
            float c = cfg.uct_c;
            assert_true( parent->children == children );
            assert_false( (parent->actions) );      //Has been fully expanded
            if( not children.size() ) return;
