...
void testBasic(){
    ...
    parser.tokenize("((<= (next (cell ?x ?y ?p)) (does ?player (move ?piece ?x1 ?y1 ?x2 ?y2)) ))", rule);
    Game* game = new Game();
    ...
    auto asserter = [&](const std::unordered_map<ushort, ares::UniqueClauseVec *> &rules_,std::vector<std::string>& tokens,ushort name){
...
void testOr(){   
    vector<string> rule;
    parser.tokenize("( (<= (legal ?player (move ?piece ?u ?v ?x ?y))\
    (true (control ?player))\
    (or (true (check ?player rook ?tx ?ty))\
        (true (check ?player queen ?tx ?ty)))) )", rule);
    
    Game* game = new Game();
    parser.parse(game,rule);
    const auto* ucv = (*game)[Namer::LEGAL];
    assert_true(ucv )
    assert_true( ucv->size() == 1);
    const auto* clause = (*ucv)[0];
    assert_equal( clause->getHead()->to_string(), "(legal ?player (move ?piece ?u ?v ?x ?y))");
    assert_equal( clause->getBody()[0]->to_string(), "(true (control ?player))");
    assert_equal( clause->getBody()[1]->to_string(), "(or (true (check ?player rook ?tx ?ty))(true (check ?player queen ?tx ?ty)))");
...
void testNot(){   
    ...
    (not (occupied_by_player ?x ?y ?player))) )", rule);
    ...
    assert_equal( clause->getHead()->to_string(), "(legal ?player (move ?piece ?u ?v ?x ?y))");
    assert_equal( clause->getBody()[0]->to_string(), "(or (true (check ?player bishop ?tx ?ty))(true (check ?player queen ?tx ?ty)))");
    assert_true( clause->getBody()[0]->get_type() == Term::OR);
    assert_equal( clause->getBody()[1]->to_string(), "(not (occupied_by_player ?x ?y ?player))");
...
void testNested(){
    ...
    (or (not (true (check ?player bishop ?tx ?ty)))\
        (true (check ?player queen ?tx ?ty)))\
    (not (or  (true (check ?player queen ?tx ?ty)) (occupied_by_player ?x ?y ?player) )) ))", rule);
    Game* game = new Game();
    parser.parse(game,rule);
    const auto* ucv = (*game)[Namer::LEGAL];
    assert_true(ucv )
    ...
    assert_equal( clause->getHead()->to_string(), "(legal ?player (move ?piece ?u ?v ?x ?y))");
    assert_equal( clause->getBody()[0]->to_string(), "(or (not (true (check ?player bishop ?tx ?ty)))(true (check ?player queen ?tx ?ty)))");
    assert_true( clause->getBody()[0]->get_type() == Term::OR);
    assert_equal( clause->getBody()[1]->to_string(), "(not (or  (true (check ?player queen ?tx ?ty)) (occupied_by_player ?x ?y ?player) ))");
    assert_true( clause->getBody()[1]->get_type() == Term::NOT);
}