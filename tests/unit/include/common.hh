...
void setup()
{
    ...
    srand(time(NULL));
    aresP.mempool =
        &MemoryPool::create(100, 100, std::vector<std::pair<arity_t, uint>>());
...
namespace ares
{
    ...
    Body* getRandBody(OrdrdVarSet& vars, ushort arity, ushort& depth,
                      ushort max_arity = 3)
    {
    ...
    lit_var_pair getRandAtom(ushort depth, ushort max_arity = 3,
                             ushort max_arity_sb = 3)
    {
        PoolKey key;
    ...
    void extractVar(const structured_term* l, OrdrdVarSet& v)
    {
        for (auto&& i : l->getBody()) {
            ...
            else if (is_struct_term(i))
                extractVar((structured_term*)i, v);
        }
    }
};  // namespace ares
