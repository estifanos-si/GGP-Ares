...
namespace ares
{
    ...
    const structured_term* GdlParser::parseSterm(const char* st,
                                                 const Creator& crtr)
    {
    ...
    {
        ifstream f(gdlF);
        string gdl((istreambuf_iterator<char>(f)), istreambuf_iterator<char>());
        gdl = "(" + gdl + ")";
    ...
    void GdlParser::parse(vector<string>& tokens, KnowledgeBase* base)
    {
        ...
        for (it = tokens.begin() + 1; it < tokens.end();) {
            switch (state) {
                case NEW:
                    if (it >= tokens.end())
                        throw SyntaxError(
                            "[*] GdlParser ::Error:: Gdl not well formed! "
                            "Expecting a ')' at end.");
                    if ((it + 1) == tokens.end()) {
                ...
                case BALANCE:
                    ...
                    if (*(start + 1) != "<=") {
                        ...
                        pool->post([this, start, end, c, &tokens, base]() {
        ...
        if (state != END)
            throw SyntaxError("[*] GdlParser ::Error:: Gdl not well formed!");
    }
    ...
    string GdlParser::preprocess(string& gdl)
    {
        ...
        auto result =
            boost::regex_replace(gdl, re_b, [](const boost::smatch& match) {
                for (auto&& c : match.str())
                    if (!isspace(c))
                        return string(" ") + string(1, c) + string(" ");
    ...
    const structured_term* GdlParser::parseSterm(
        vector<string>::iterator& start, const vector<string>::iterator& end,
        const Creator& crtr, bool p)
    {
        stack<pair<string, Body*>> bodies;
        if (*start != "(") {
            // Its a literal without  a body like 'terminal'
            if (start >= end)
                throw SyntaxError("GDLParser :: Error :: Atom start > end");
        ...
        while (!bodies.empty() and it < end) {
            ...
            if (isalnum(token[0])) {
                ...
                const Constant* cnst =
                    memCache->getConst(Namer::registerName(token));
                body->push_back(cnst);
            } else if (token[0] == '?' and token.size() > 1) {
                auto& body = bodies.top().second;
                const Variable* var =
                    memCache->getVar(Namer::registerVname(token));
                body->push_back(var);
            } else if (token == "(") {
                ...
                // a well formed expression should eventually get into the next
                // if.
                if ((l = create(ref(bodies), crtr, p)))
                    return l;
            } else
                throw SyntaxError("GdlParser :: Error :: Unexpected token : " +
                                  token + " \nWhile parsing " + name);
        ...
        throw UnbalancedParentheses(
            "GdlParser :: Error :: Unbalanced parantheses while parsing "
            "literal " +
            name);
    }
    const structured_term* GdlParser::create(stack<pair<string, Body*>>& bodies,
                                             const Creator& crtr, bool)
    {
        ...
        // You can do further check to ensure Function and literal names are
        // distinct! if necessary.
        const Function* fn = memCache->getFn(key);
        bodies.top().second->push_back(fn);
        return nullptr;
    }
    void GdlParser::parseRule(Clause* c, KnowledgeBase* base, vector<string>&,
                              vector<string>::iterator start,
                              const vector<string>::iterator end)
    {
        //*start == "(" , *end == ")" and *(start+1) == "<=" checked in
        //parse(...) above
        start += 2;
        // Get the head
        ...
        if (start >= end)
            throw SyntaxError("GdlParser :: Error :: Empty Rule Body");
        // populate the body by applying transformations as necessary
        // TokenStream* stream = new TokenStream(tokens, start, start, end);
        // transformer->applyTransformations(c,
        // base,unique_ptr<TokenStream>(stream)); parseBody(start, end,
        // c->getBody())
        parseBody(start, end, c->getBody());
        base->add(head->get_name(), c);
    }
    void GdlParser::parseBody(vector<string>::iterator& begin,
                              const vector<string>::iterator& end, Body& body)
    {
        if (begin == end) {
            if (*end != ")")
                throw SyntaxError("GdlParser :: Expecting a ')'");
            return;
        }
        if (*begin != "(")
            // Atom without a body
            ...
            if (it >= end)
                throw SyntaxError("GdlParser :: end reached before a ')'");
            begin += 2;
