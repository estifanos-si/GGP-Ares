...
namespace ares
{
    ...
    MemoryPool::MemoryPool(std::size_t st_terms, std::size_t clause_s,
                           std::vector<std::pair<arity_t, uint>> arites)
    {
        ...
        POOLS = std::vector<std::vector<void*>*>{&st_term_pool, &clause_pool,
                                                 &body_pool};
    ...
    MemoryPool::~MemoryPool()
    {
        ...
        for (std::vector<void*>* v : POOLS)
            for (void* vp : *v) free(vp);

        for (auto&& it : container_pool)
            for (void* vp : it.second.second) delete ((term_container*)vp);

        delete EMPTY_CONTAINER;
    }
    void MemoryPool::init_pools(std::size_t st_terms, std::size_t clause_s,
                                std::vector<std::pair<arity_t, uint>> arities)
    {
        /* Pre allocate term containers i.e vector<shared_ptr<Term>>, of given
         * arities*/
        init_pool(arities);
    ...
    /**
     * @returns a container<cnst_term_sptr>* or container<cnst_lit_sptr>* from
     * the free pool
     */
    void* MemoryPool::allocate(arity_t arity)
    {
        ...
        if (pool.size() == 0)
            grow(pool, ar_gfactor(arity), arity);
        return allocate(pool);
    }
    /**
