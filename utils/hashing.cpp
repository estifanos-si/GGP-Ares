...
namespace ares
{
    ...
    void UniqueVector<T, Hash, Eq>::apply(
        bool rand,
        const std::function<void(std::reference_wrapper<const T>)> op) const
    {
        UIterator<T> it =
            rand ? RandIterator<T>(elements) : UIterator<T>(elements);
        while (it) {
            op(*it);
            ++it;
        }
    ...
    std::size_t ClauseHasher::operator()(const Clause* c) const
    {
        std::size_t hash = 0;
        if (c->head)
            hash_combine(hash, c->getHead());
        for (auto&& lit : *c->body) hash_combine(hash, lit);

        return hash;
    }
    bool ClauseHasher::operator()(const Clause* c, const Clause* c2) const
    {
        if (c->body->size() != c2->body->size() || (c->head != c2->head))
            return false;
    ...
    {
        std::size_t nHash = 0;
        for (auto& t : *k.body) hash_combine(nHash, t);
        return nHash;
    }
    bool PoolKeyHasher::equal(const PoolKey& k1, const PoolKey& k2) const
    {
        if ((k1.body->size() != k2.body->size()))
            return false;
