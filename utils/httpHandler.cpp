...
namespace ares
{
        ...
        : ares(ares_), listener(url), hooks(5), playing(false), seq(0)
    {
        ...
        hooks[INFO] = [&](std::vector<std::string>&) {
            ...
            return std::string("( (name Ares) (status ") +
                   status;  // Return info
        };
        ...
        hooks[PLAY] = [this](std::vector<std::string>& s) {
            return playHandler(s);
        };
        ...
        // Start up the server
        listener.support(methods::POST, std::bind(&HttpHandler::handle, this,
                                                  std::placeholders::_1));
        listener.open()
            .then([&] {
                log("\n[HttpHandler]")
                    << "Ares Server Started on Adress : " << url << "\n";
            })
            .wait();
    }
    void HttpHandler::handle(http_request msg)
    {
        ...
        msg.extract_string()
            .then([&](pplx::task<string> task) {
                ...
                // Don't log pings.
                if (cfg.debug or t != INFO)
                    log("[HttpHandler]")
                        << "Got a Post Message : " << sMsg << "\n";
                if (t == -1) {
                    msg.reply(status_codes::BadRequest);
                    return;
                }
                // One of the info, start, play, or stop messages; handle them
                // accordingly.
                auto& hook = hooks[t];
                auto reply = hook(tokens);
                if ((cfg.debug or t != INFO) and reply.size())
                    log("[HttpHandler]") << "HttpReply : " << reply << "\n";
                else if (not reply.size())
                    logerr("[HttpHandler::Handler]") << "Empty HttpReply."
                                                        "\n";
    ...
    std::string HttpHandler::playHandler(std::vector<std::string>& tokens)
    {
        ...
        uint cseq = ++seq;
        log("[HttpHandler]")
            << "Recieved Play message for match : " << tokens[2] << '\n';
        ...
        if (not valid) {
            if (selectedMove.second != seq)
                logerr("[HttpHandler]")
                    << "Strategy returned invalid move.\n\tcurrent seq=" << seq
                    << ", returned seq = " << selectedMove.second << "\n";
            else
                logerr("[HttpHandler]") << "Strategy returned invalid move. "
                                           "Returned move is null.\n";
        }
