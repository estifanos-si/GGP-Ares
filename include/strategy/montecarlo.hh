    ...
    class Montecarlo : public Strategy, public RegistrarBase<Montecarlo>
    {
     public:
        ...
        struct Node {
            ...
            ActionIterator
                actions;  // The available actions from this nodes state
            std::vector<Node*> children;
            ucAction action;  // The action that was taken from the parents
                              // state
            std::mutex lock;
     ...
     public:
        ...
        static Montecarlo* create()
        {
            static Montecarlo monte;
            return &monte;
        }
        ...
        inline static float uct(const Node& nd, ushort c, ushort p, float def)
        {
            return nd.n == 0
                       ? def
                       : (nd.values[p] / nd.n) +
                             (c * sqrt((2 * std::log(nd.parent->n)) / nd.n));
        }
        /**
        ...
        inline static Node* bestChild(Node& n, ushort c, ushort role, float def)
        {
            ...
            return *std::max_element(
                n.children.begin(), n.children.end(), [&](auto& n1, auto& m) {
                    return uct(*n1, c, role, def) < uct(*m, c, role, def);
                });
        }
     ...
     private:
        ...
        inline void update(Node* v, std::vector<float> vals)
        {
            while (v) {
                ...
                // otherwise the parent of v depends on v's value for bestChild
                // selection
                std::lock_guard<std::mutex> lk(v->parent ? v->parent->lock
                                                         : tree.lock);
                v->n++;
                for (uint i = 0; i < vals.size(); i++) v->values[i] += vals[i];
     ...
     private:
        ...
        struct Tree {
            ...
            Montecarlo& mc;
            std::mutex lock;
            Node* root;      // Keep track of current state. This dynamically
                             // changes on every step.
            Node* origRoot;  // For debugging and visualization purposes don't
        ...
        class Timer
        {
         public:
            ...
            std::future<const Term*> reset(std::atomic_bool& done_,
                                           const Match& match);
            /**
     ...
     public:
        struct ISelectionPolicy {
            virtual Node* operator()(Node*, const std::atomic_bool&,
                                     Montecarlo::Ptype, ushort indx) const = 0;
            virtual ~ISelectionPolicy() {}
        };
        struct ISimPolicy {
            virtual std::vector<float> operator()(const State* state,
                                                  std::atomic_bool&) const = 0;
            virtual ~ISimPolicy() {}
        };
        /**
         * Data
     ...
     private:
        ...
        Timer timer;
        std::mutex lock;
        ISelectionPolicy* selPolicy;
        ISimPolicy* simPolicy;
