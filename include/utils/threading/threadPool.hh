    ...
    class ThreadPool
    {
     ...
     public:
        ...
        inline void wait()
        {
            ...
            loadBalancer->cvCheckEmpty.wait(
                lk, [this]() { return loadBalancer->noJobs(); });
            // stop();//for debugging purposes
        }
        /**
         * No more jobs are scheduled after this point, untill pool is
         * re-started using restart().
         */
        inline void stop()
     ...
     private:
        ...
        bool finished = false;  // Workers constantly check this to know if they
                                // should exit processing loop.
    };

    class ThreadPoolFactroy
    {
     private:
        typedef tbb::concurrent_hash_map<ushort, std::vector<ThreadPool*>>
            PoolMap;
        ThreadPoolFactroy() {}

     public:
        inline static ThreadPoolFactroy& create()
        {
            static ThreadPoolFactroy tpf;
            return tpf;
        }
        inline static ThreadPool* get(ushort nThreads)
        {
            ThreadPool* p;
            {
                ...
                if (pools.insert(ac, nThreads) or !ac->second.size())
                    ac->second.push_back(
                        new ThreadPool(new LoadBalancerRR(nThreads)));
            ...
            while (!finished) {
                {
                    ...
                    cvOutstdWork.wait(
                        lck, [this]() { return (!jobs.empty()) or finished; });
                    if (finished)
                        return;
                }
