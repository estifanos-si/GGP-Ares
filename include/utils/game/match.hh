...
namespace ares
{
    ...
    class State : public KnowledgeBase
    {
     ...
     public:
        ...
        std::unordered_map<ushort, UniqueClauseVec*>::const_iterator begin()
            const
        {
            return state.cbegin();
        }
        ...
        std::string toStringHtml() const
        {
            std::string s;
            for (auto&& [name, vec] : state) {
                for (auto&& i : *vec) s.append(i->to_string() + "<br/>");
            }
            return s;
        }
        bool operator==(const State& other)
        {
            ...
            for (auto&& [name, vec] : state) {
                ...
                if (it == other.state.end() or ((*vec) != (*it->second)))
                    return false;
            }
            return true;
        }
