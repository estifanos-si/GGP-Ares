...
namespace ares
{
    ...
    class GdlParser
    {
        typedef std::function<const structured_term*(
            std::reference_wrapper<PoolKey>)>
            Creator;
     ...
     private:
         ...
         */
        const structured_term* parseSterm(vector<string>::iterator& start,
                                          const vector<string>::iterator& end,
                                          const Creator&, bool p = true);
        const structured_term* parseSterm(const char*, const Creator& crtr);
        /**
         * Just a wrapper function.
         */
        inline const Atom* parseAtom(vector<string>::iterator& start,
                                     const vector<string>::iterator& end,
                                     bool p = true)
        {
        ...
        inline const Function* parseFn(vector<string>::iterator& start,
                                       const vector<string>::iterator& end)
        {
         ...
         */
        void parseRule(Clause* c, KnowledgeBase* base, vector<string>& tokens,
                       vector<string>::iterator start,
                       const vector<string>::iterator end);
        void parseBody(vector<string>::iterator& begin,
                       const vector<string>::iterator& end, Body& body);
        const structured_term* create(stack<pair<string, Body*>>& bodies,
                                      const Creator&, bool p = true);
     ...
     public:
        ...
        ~GdlParser() { ThreadPoolFactroy::deallocate(pool); }
    ...
    inline void checkValid(string& s)
    {
        ...
        if (s.find('(') != s.npos or s.find(')') != s.npos or (s == "or") or
            (s == "not"))
            throw SyntaxError(
                "[*] GdlParser ::Error:: A literal/term Name should only "
