...
namespace ares
{
    ...
    class Clause
    {
     ...
     public:
        ...
        static bool EMPTY_CLAUSE(const Clause& c)
        {
            return (c.body->size() == 0 and (not c.head));
        }

        inline Clause* clone(bool ctheta = false) const
        {
            auto* c =
                new Clause(head, new ClauseBody(body->begin(), body->end()));
            if (ctheta and theta)
        ...
        inline Clause* next() const
        {
            ...
            auto* c = new Clause(
                head, new ClauseBody(body->begin() + n, body->end()));
            if (theta)
                c->setSubstitution(new Substitution(*theta));
            return c;
        }
        /**
         * @brief Create a new renamed clause, used in a resolution step while
         * resolving a goal.
        ...
        void delayFront() { body->front_to_back(); }
        ...
        std::string to_string() const
        {
            ...
            if (body->size() > 0)
                s.append("(<= ");
            if (head)
                s.append(head->to_string() + " ");

            for (auto& l : *body) s.append("\n" + l->to_string());
