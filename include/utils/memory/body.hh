...
namespace ares
{
    ...
    template <class T>
    struct _Body {
        typedef const typename std::vector<T*>::const_iterator
            const_vec_iterator;
     ...
     private:
        ...
        /**
         * Same as _Body _Body(const_vec_iterator& begin, const_vec_iterator&
         * end) but with no arity check for zero.
         */
        _Body(const_vec_iterator& begin, const_vec_iterator& end, bool)
            : _Body()
        {
        ...
        void push_back(T* t)
        {
            if ((const lit_container*)container ==
                MemoryPool::EMPTY_CONTAINER) {
                int i;
     ...
     public:
        ...
        {
            front_ = 0;
            // no need to allocate a container of arity 0 it should already be
            // constructed by MemPool
            if (arity_ == 0)
        ...
        void operator delete(void* p) { mempool->deallocate((_Body<T>*)p); }

        ~_Body()
        {
            const auto& arity = container->size();
            for (uint i = front_; i < arity; i++) (*container)[i] = nullptr;
        ...
        T*& operator[](std::size_t i) const
        {
            i += front_;
            if (i >= container->size())
                throw IndexOutOfRange("Body size out of range size is : " +
                                      std::to_string(size()) +
        ...
        const_vec_iterator end() const { return container->cend(); }
        void front_to_back()
        {
            if (this->size() <= 1)
                throw EmptyBodyPop(
                    "front_to_back called on a body of size <= 1.");
        ...
        {
            if (this->size() == 0)
                throw EmptyBodyPop("pop_front called on an empty body.");
     ...
     private:
        /**
         * Keep track of the front during pops rather than actually deleting the
         * element which makes the body not reusable.
         */
        std::size_t front_ = 0;
