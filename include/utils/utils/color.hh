...
namespace rang
{
    /* For better compability with most of terminals do not use any style
     * settings except of reset, bold and reversed. Note that on Windows
     * terminals bold style is same as fgB color.
     */
    enum class style {
        reset = 0,
        bold = 1,
    ...
    namespace rang_implementation
    {
...
#elif defined(OS_WIN)
            // All windows versions support colors through native console
            // methods
        ...
        inline bool isMsysPty(int fd) noexcept
        {
            ...
            if (!ptrGetFileInformationByHandleEx(h, FileNameInfo,
                                                 pNameInfo.get(),
        ...
        struct SGR {  // Select Graphic Rendition parameters for Windows console
            BYTE fgColor;  // foreground color (0-15) lower 3 rgb bits + intense
                           // bit
            BYTE bgColor;  // background color (0-15) lower 3 rgb bits + intense
                           // bit
        ...
        enum class AttrColor : BYTE {  // Color attributes for console screen
                                       // buffer
            black = 0,
            red = 4,
            green = 2,
        ...
        inline HANDLE getConsoleHandle(const std::streambuf *osbuf) noexcept
        {
            if (osbuf == std::cout.rdbuf()) {
                ...
                return hStdout;
            } else if (osbuf == std::cerr.rdbuf() ||
                       osbuf == std::clog.rdbuf()) {
