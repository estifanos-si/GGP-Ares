...
{
    struct AnswerList {
        AnswerList() : qi(0) { newSolns.share(solns); }
        ...
        inline AnswerList* addAnswer(const Atom* lit, const Substitution& ans)
        {
            VarSet vset;
            auto soln = (*lit)(ans, vset);
            if (newSolns.push_back((const Atom*)soln))
                return this;
            return nullptr;
        }
        /**
         * Mainly used for restarting suspended queries, when an answer has been
         * found.
         */
        template <class T>
        inline void apply(T op)
        {
            ...
            for (auto&& query : observers[j]) op(query);
     ...
     private:
        ...
        UniqueVector<const Atom*, AtomHasher, AtomHasher>
            solns;  // Answers found this far
        UniqueVector<const Atom*, AtomHasher, AtomHasher>
            newSolns;                                      // new Answers
    ...
    struct Next {
        ...
        inline void apply(T op)
        {
            ...
            // Include the new solutions
            for (auto&& ansL : next[j]) ansL->next();

            // Restart susspended queries, if any
            for (auto&& ansL : next[j]) ansL->apply(op);
     ...
     private:
        UniqueVector<AnswerList*> next[2]{UniqueVector<AnswerList*>(),
                                          UniqueVector<AnswerList*>()};
        SpinLock slck;
        ushort i;
    };
    /**
    ...
    class Cache
    {
        typedef tbb::concurrent_hash_map<
            const Atom*, std::unique_ptr<AnswerList>, AtomHasher>
            AnsCache;

     private:
        tbb::concurrent_hash_map<const Atom*, std::unique_ptr<AnswerList>,
                                 AtomHasher>
            ansCache;
        // std::unordered_set<cnst_lit_sptr> failCache;
     public:
        Cache(AnsIterator::Type ansType_ = AnsIterator::SEQ) : ansType(ansType_)
        {
        }
        /**
        ...
        inline AnsIterator operator[](Query& q)
        {
            ...
            if (ansCache.insert(ac, (const Atom*)q->front())) {
            ...
            nlookups++;
            return ansType == AnsIterator::SEQ ? ansIt
                                               : RandomAnsIterator(ansIt);
        }
        /**
         * insert a new answer to the answer list of l.
         */
        inline bool addAns(const Atom* l, const Substitution& ans)
        {
            AnsCache::accessor ac;
            if (!ansCache.find(ac, l))
                throw "Tried to insert solution to a non existent key!";
            if (auto ansl = ac->second->addAnswer(l, ans)) {
                // Found a new answer, register answerlist so that we can
                // restart suspended queries
