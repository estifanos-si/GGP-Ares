...
namespace ares
{
    ...
    class Reasoner
    {
     ...
     public:
         ...
         * @param action an ordered (by role order) list of moves taken by
         * roles. i.e action[i] is the move taken by role[i].
         ...
         */
        virtual Moves* moves(const State& state, const Role& role,
                             bool rand = false);
        /**
        ...
        virtual inline ushort roleIndex(ushort name) const
        {
            return rolesIndex.at(name);
        }
        /**
         * Get a random move.
         */
        ...
        /**
         * Get a random action, i.e <move_1,...,move_n> , where move_i is taken
         * by role i.
         */
     ...
     private:
         ...
         */
        void query(uClause& goal, const State* context, SharedCB cb,
                   bool rand = false);

        /**
         * For ease of access to legal and goal queries of the form (legal
         * some_role ?x)/(goal some_role ?x). initializes roleLegalMap and
         * roleGoalMap.
         */
        void initMapping();
         ...
         */
        inline void getCombos(std::vector<uAction>& legals, uint i,
                              Moves& partials, std::vector<uAction>& combos)
        {
            if (i >= legals.size()) {
                combos.push_back(
                    uAction(new Moves(partials.begin(), partials.end())));
                return;
            }
    ...
    struct NxtCallBack : public CallBack {
        NxtCallBack(Reasoner* t, State* s)
            : CallBack(done_, nullptr), this_(t), newState(s), done_(false)
        {
        }
        ...
        virtual void operator()(const Substitution& ans, ushort, bool)
        {
            ...
            VarSet vset;
            auto true_ =
                (const Atom*)(*this_->TRUE_LITERAL)(ans, vset);  // Instantiate
            if (true_) {
    ...
    struct RewardCallBack : public CallBack {
        RewardCallBack(Reasoner* t)
            : CallBack(done_, nullptr), reward(0.0), this_(t), done_(false)
        {
        }
        virtual void operator()(const Substitution& ans, ushort, bool)
        {
