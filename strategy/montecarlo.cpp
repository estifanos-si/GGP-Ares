...
namespace ares
{
    ...
    std::pair<Move*, uint> Montecarlo::operator()(const Match& match, uint seq)
    {
        ...
        if (stoped) {
            timer.cancel();
            return std::pair<Move*, uint>();
        }
        if (match.takenAction) {
            tree.select(
                match.takenAction);  // change the root according to made moves
            current = tree.root->state.get();
        }
        struct search {
            ...
            void operator()()
            {
                while (not done) {
                    auto* v =
                        (*mc.selPolicy)(mc.tree.root, done, mc.player, indx);
                    if (id == 0 and cfg.debug)
                        mc.dump();
        ...
        player = (player + 1) %
                 (order.size() ? order.size()
                               : 1);  // switch player according to thier turn
        return std::pair<Move*, uint>(future.get(), seq);
    }

    Montecarlo::Node* SelectionPolicy::operator()(Montecarlo::Node* n,
                                                  const std::atomic_bool& done,
                                                  Montecarlo::Ptype p,
                                                  ushort indx) const
    {
        while ((not done) and not reasoner.terminal(*n->state)) {
            // log("[Montecarlo]") << "Current node is " << "player " << p
            // <<"s.\n" ;
            if (auto* a = n->actions.next()) {  // is there an unvisited state
                                                // reachable from here?
                // expand the node
                auto* child =
                    new Montecarlo::Node(reasoner.next(*n->state, *a), a);
                n->add(child);
                child->parent = n;
                return child;
            } else
                n = Montecarlo::bestChild(*n, cfg.uct_c,
                                          Montecarlo::plyr(p, indx), INFINITY);
        }
        return n;
    }
    std::vector<float> SimPolicy::operator()(const State* state,
                                             std::atomic_bool& done) const
    {
     ...
     */
    inline void Montecarlo::Node::erase()
    ...
    /**
     * make the node that is a child of root and whose action == action the new
     * root.
    ...
    inline void Montecarlo::Tree::select(const Action* action)
    {
        ...
        if (not selected) {
            ...
            // need to construct a new tree with root having state =
            // next(root.state, action);
            selected =
                new Node(mc.reasoner->next(*root->state, *action), nullptr);
            root->add(selected);
        }
     ...
     * Timer methods
     */
    inline std::future<const Term*> Montecarlo::Timer::reset(
        std::atomic_bool& done_, const Match& match)
    {
        std::lock_guard<std::mutex> lk(lock);
        auto duration = (match.plyClck * 1000) - cfg.delta_milli;
        uint cseq = ++seq;
        if (done) {
            logerr("[Montecarlo::Timer]")
                << " reset during simulation. Reseting previous simulation.\n";
            *done = true;
        }
        log("[Montecarlo::Timer]")
            << "Reseting timer to " << duration << " milliseconds\n";
        done = &done_;
        cv.notify_all();  // Cancel previous searches (if any)
        auto cb = [cseq, this, &done_, role(match.role->get_name()),
                   duration]() -> const Term* {
            {
                std::unique_lock<std::mutex> lk(lock);
                cv.wait_for(lk, std::chrono::milliseconds(duration),
                            [&] { return (bool)done_; });
            }
            done_ = true;  // If time has expired cancel the search
            log("[Montecarlo::Timer]")
                << "Waking up for sequence number " << cseq << "\n";
            if (cseq != seq)
                return nullptr;
            Node* node;
            auto i = mct.reasoner->roleIndex(role);
            {
                ...
                if (not root)
                    return nullptr;
                node = root->children.size()
                           ? mct.bestChild(*root, 0, (Montecarlo::Ptype)i, 0)
                           : nullptr;
            }
            if (!node)
                return nullptr;
    ...
    Montecarlo::~Montecarlo() { setPolicies(nullptr, nullptr); }

    /**
     * Utilities
     */
    inline void dump_(Montecarlo::Node* root, Montecarlo::Node* n,
                      std::string& treeSt)
    {
        ...
        for (uint i = 0; i < n->values.size(); i++) {
            float uctv = (!n->parent)
                             ? 0
                             : Montecarlo::uct(*n, cfg.uct_c,
                                               (Montecarlo::Ptype)i, INFINITY);
            treeSt += sep + "\"" + to_string(uctv) + "\"";
            sep = ",";
        }
        treeSt += "],";
        // float uctv1 = (!n->parent) ? 0 :
        // Montecarlo::uct(*n,cfg.uct_c,(Montecarlo::Ptype)1,INFINITY); treeSt
        // += R"#(   "uct0": ")#" + to_string(uctv0) + "\","; treeSt += R"#(
        // "uct1": ")#" + to_string(uctv1) + "\",";
        treeSt += R"#("children" : [)#";
        sep = "";
    ...
    void Montecarlo::dump(std::string)
    {
        std::string treeSt;
        if (tree.root)
            dump_(tree.root, tree.origRoot, treeSt);
        else
            treeSt = "{}";
