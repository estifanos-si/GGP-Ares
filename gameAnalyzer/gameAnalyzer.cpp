...
namespace ares{
    GameAnalyzer::GameAnalyzer(Reasoner& reasoner_):reasoner(reasoner_),done(false)
    {
    ...
    UniqueVector<ushort> GameAnalyzer::isAZSG(const Game* game,uint startClk){
        log("[GameAnalyzer]")<< "Checking Alternating Zero-Sum Property Via Simulations.\n";
        done = false;
        UniqueVector<ushort> order;
        if( game->roles().size() == 1 ) return order;

        const auto* init = game->init();
        auto deleter = [&](const State* s) { if (s != init ) delete s;};
        ...
        std::thread timer([&]{
            auto milli = (startClk * 1000) - cfg.delta_milli;
            {
                ...
                log("[GameAnalyzer]") << "Timer set for " << milli << " milliseconds.\n";
                cv.wait_for(lk,std::chrono::milliseconds(milli),[&]{ return (bool)done;});
            }
            done = true;
        });
        ...
        if( not azsg ) {std::cout << "NOT Alternating Zero-Sum game.\n"; order.clear();}
        else{ 
            ...
            {
                str += sep + "Player " + to_string(i+1) + " : " + reasoner.roles()[order[i]]->to_string();
                sep=",";
            }
            log( str ) << "\n";
        }
    ...
    void GameAnalyzer::checkAlt(uState state,UniqueVector<ushort>& order,std::atomic_bool& azsg){
        const auto roles = reasoner.roles();
        ...
        Fn check =[&](uState state){
            if( done or not azsg ) {
                if( ambcount/count  >= 0.09 ) azsg = false;
                return;
            }
            ...
            {
                if( reasoner.terminal(*state) or done ) break;
                ...
                if(( player == -1 )) ambigious = true;
                else if(  not turn.push_back(player)){
                    azsg = false;  //player got more than 1 turn within an n-consecutive plays/sates
                    break;
                }
                //compute next state
            ...
            if( ambigious ) ambcount++;
            if( reasoner.terminal(*state) or done ) {
                if( count and (ambcount/count  >= 0.34) ) azsg = false;
                return;
            }
            if( turn.size() ){
                if( not order.size() ) order = turn;
